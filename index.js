...
const publicChatSchema = require('./PublicChatDB');
...
mongoose.connection.on('connected', () => {
  ...
  const publicChatDb = mongoose.connection.useDb('PublicChat');  // Fix db reference to PublicChat database
    const PublicChat = publicChatDb.model('PublicChat', publicChatSchema, 'ChatData');
    console.log('PublicChat model initialized');
});
...
app.get('/publicchat', async (req, res) => {
  try {
    const comments = await PublicChat.find().sort({ date: -1 });
    ...
    // If data exists, return the comments
    res.status(200).json({
    ...
    res.status(500).json({
      message: 'Server error. Unable to fetch comments.',
    });
  }
});

app.post('/publicchat', async (req, res) => {
  ...
  try {
    const newComment = new PublicChat({ content, email, userName });
    const savedComment = await newComment.save();
    res.status(201).json(savedComment);
  } catch (error) {
    console.error('Error posting comment:', error);
    res.status(500).json({ message: 'Server error. Failed to post comment.' });
  }
});
